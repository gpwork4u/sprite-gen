#!/usr/bin/env python3
"""Vendored from agent-sprite-forge (MIT, Copyright (c) 2026 0x0funky).

Source: https://github.com/0x0funky/agent-sprite-forge
       skills/generate2dsprite/scripts/generate2dsprite.py

Reused unmodified for chroma-key cleanup, frame extraction, alignment, QC, and
GIF export. See third_party/agent-sprite-forge-LICENSE for the full license.

pixelforge imports helper functions (remove_bg_magenta, split_grid, compose_sheet,
save_transparent_gif) from this module rather than running it as a CLI."""
...
def save_transparent_gif(frames: list[Image.Image], out_path: Path, duration: int) -> None:
    ...
    for index, canvas in enumerate(visible_rgbs):
        stacked.paste(canvas, (0, index * height))
